...
- openspec/                   # OpenSpec 规范目录
    ...
    - changes/                # 变更提案目录
        - refactor-watermark-editor-ui/  # 当前重构提案（进行中）
        - archive/            # 已归档的变更
    - project.md              # 项目上下文文档
    - specs/                  # 功能规范目录
- public/                     # 静态资源目录
...
- src/                        # 源代码目录 (核心!)
    ...
    - types/
        - watermark.ts        # 水印相关类型定义（重要! 包含 TextBox, TileWatermarkConfig）
    - utils/
    ...
    - components/
        - ui/                 # shadcn/ui 组件
            ...
            - switch.tsx      # 新增：开关组件
        - Toolbar/            # 工具栏组件
            - FloatingToolbar.tsx  # 新增：浮动工具栏（选中文本框时显示）
            - TileConfigPanel.tsx  # 新增：平铺水印配置面板
            - Toolbar.tsx     # 旧版工具栏（待清理）
            - ModeSelector.tsx # 旧版模式选择器（待清理）
            - ColorPicker.tsx # 颜色选择器
            - SliderControl.tsx # 通用滑块控件
            - PositionSelector.tsx # 旧版位置选择器（待清理）
        - Canvas/             # Canvas 相关组件
            - ImageCanvas.tsx # Canvas 预览和渲染（重要! 已重构）
            - ImageUploader.tsx # 图片上传组件（已简化）
        - Export/
            - ExportButton.tsx # 导出按钮组件（待整合）
        - WatermarkEditor.tsx # 主编辑器组件（已重构，新架构）
    - layouts/
        - Layout.astro        # 基础布局组件
    - pages/
        - index.astro         # 首页（入口页面）
